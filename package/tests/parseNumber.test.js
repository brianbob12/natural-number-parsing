...
describe("parseNumber different number formats", () => {
	...
	it("should be able to parse natural language decimal million/billion/trillion", () => {
		...
		expect(parseNumber("forty two point four billion")).toBe(
			42.4 * 1_000_000_000,
		)
		expect(parseNumber("forty two point four trillion")).toBe(
			42.4 * 1_000_000_000_000,
		)
	})

	it('should be able to parse a number with "k" for thousand', () => {
		expect(parseNumber("42k")).toBe(42000)
		expect(parseNumber("3k")).toBe(3000)
		expect(parseNumber("42.4k")).toBe(42400)
		expect(parseNumber("42.42k")).toBe(42420)
		expect(parseNumber("42k five hundred and seventy")).toBe(42570)
		expect(parseNumber("42k 570")).toBeNaN() //this is considered two numbers
	})
