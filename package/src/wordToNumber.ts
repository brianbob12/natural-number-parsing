...
const isHyphenatedNumber = (word:string):null|[string,string] => {
  ...
  if( ONES.indexOf(splits[1]) <= 1 ){ //word-zero is not a split
    return null
  }
  if( TENS.indexOf(splits[0]) <= 1 ){ //word-zero is not a split
    return null
  }
...
export const wordToNumber = (input:string):[number|null,boolean] => {
  ...
  if(words.length > 0 && 
    (words[0] === "minus" || words[0] === "negative")){
    words.shift()
